      ...
      {!report ? (
        ...
        <>
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {[
              { icon: Users, label: "People Reached", value: report.counts.reached, color: "#5f7a52" },
              { icon: Target, label: "New Converts", value: report.counts.accepted, color: "#4d7c0f" },
              { icon: Baby, label: "Baptisms", value: report.counts.baptized, color: "#7a9470" },
              { icon: HandHeart, label: "Answered Prayers", value: report.counts.answeredPrayers, color: "#a8b98e" },
              { icon: Award, label: "Follow-ups Completed", value: report.counts.completed, color: "#4d7c0f" },
              { icon: XCircle, label: "Missed", value: report.counts.missed, color: "#b3261e" },
              { icon: Flame, label: "Cancelled", value: report.counts.cancelled, color: "#b45309" },
              { icon: TrendingUp, label: "Active Follow-ups", value: report.counts.pending, color: "#b45309" },
            ].map((c) => (
          ...
          <div className="rounded-lg border bg-card p-4">
                ...
                return (
                  <div key={m.month} className="flex flex-col items-center gap-1">
                    <div className="flex h-32 w-full items-end justify-center gap-1">
                      <div className="w-2.5 rounded-t bg-[#7a9470]" style={{ height: `${(m.reached / max) * 100}%` }} title={`reached: ${m.reached}`} />
                      <div className="w-2.5 rounded-t bg-[#4d7c0f]" style={{ height: `${(m.converts / max) * 100}%` }} title={`converts: ${m.converts}`} />
                      <div className="w-2.5 rounded-t bg-[#b45309]" style={{ height: `${(m.baptisms / max) * 100}%` }} title={`baptisms: ${m.baptisms}`} />
                    </div>
            ...
            <div className="mt-2 flex gap-4 text-[10px] text-muted-foreground">
              <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-sm bg-[#7a9470]" /> reached</span>
              <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-sm bg-[#4d7c0f]" /> converts</span>
              <span className="flex items-center gap-1"><span className="h-2 w-2 rounded-sm bg-[#b45309]" /> baptisms</span>
            </div>
          </div>
                ...
                <table className="w-full text-left text-[11px]">
                  <thead className="text-[9px] uppercase tracking-wide text-muted-foreground">
                    <tr className="border-b">
                      ...
                      <th className="py-1.5 pr-2 text-[#4d7c0f]">Done</th>
                      <th className="py-1.5 pr-2 text-[#b3261e]">Missed</th>
                      <th className="py-1.5 text-[#b45309]">Pending</th>
                    </tr>
                  </thead>
                  <tbody>
                    {report.workers.map((w) => (
                      <tr key={w.name} className="border-b border-dashed">
                        ...
                        <td className="py-1.5 pr-2 tabular-nums text-[#4d7c0f]">{w.completed}</td>
                        <td className="py-1.5 pr-2 tabular-nums text-[#b3261e]">{w.missed}</td>
                        <td className="py-1.5 tabular-nums text-[#b45309]">{w.pending}</td>
                      </tr>
                    ))}
                ...
                <div key={f.stage} className="flex items-center gap-3">
                  ...
                  <div className="h-4 flex-1 overflow-hidden rounded-sm bg-muted">
                    <div
                      className={cn(
                        "h-full rounded-sm",
                        i >= 4 ? "bg-[#4d7c0f]" : i >= 2 ? "bg-[#7a9470]" : "bg-[#9caf88]",
                      )}
